...
import { GoatCatalog, GoatPricing } from "@external/goat/goat.types";
...
export const getGoatCatalogFromSku = async (sku: string): Promise<GoatCatalog | undefined> => {
...
export const getPricingInsight = async (catalogId: string): Promise<GoatPricing[]> => {
  ...
  return data.map(d => ({
    ...
    lowestListingPrice: Number(d.availability.lowest_listing_price_cents) / 100,
  }));
}
